...
$sqlPrintRecord = mysqli_query($con, "select * from alterationtype join tempalt t on alterationtype.alterationTypeID = t.alterationTypeID where t.cusID=" . $ID);
$total = 0;

$cusIDSQL = mysqli_query($con, "SELECT MAX(alterationID) as 'alt' FROM alteration");
...
if (isset($_REQUEST['altButtons'])) {
    ...
    mysqli_query($con, "insert into tempalt (cusID, alterationTypeID) VALUES ('$ID','$altButtons')");
    header('Location: alteration.php?ID=' . $ID);
}
...
if (isset($_REQUEST['saveAlt'])) {
    ...
    $section = "Hold";
    $sectionID = 0;
    $getSection = mysqli_query($con, "select * from customerdetails join weddingpartyattr w on customerdetails.customerID = w.customerID join weddingform w2 on w.weddingID = w2.weddingID join sectionattr s on w2.section = s.name where customerdetails.customerID = '$ID'");
    ...
    mysqli_query($con, "Delete from alteration where customerID = '$ID'");
    mysqli_query($con, "update customerDetails set status='$section',histroy  =  concat(histroy,',','$section') where customerID = '$ID'");
    mysqli_query($con, "Insert into section (sectionID, sectionName, customerId, customerName, items) VALUES ('$sectionID','$section','$ID','$name','$item')");
    header('Location: ../customer/customerDetails.php?ID=' . $ID);
}


if (isset($_REQUEST['delete'])) {
    $deleteTempAlt = "delete from tempalt where alterationTypeID = {$_REQUEST['deleteID']}";
    mysqli_query($con, $deleteTempAlt);
}
$sqlCount = mysqli_query($con, "Select count(customerID) as 'c' from alteration where customerID=" . $ID);
...
if (isset($_POST['submit'])) {
    ...
    $aPrice = array();
    $aName = array();
    $query = mysqli_query($con, "Select alterationTypeID from tempalt where cusID = " . $ID);
    ...
    $sqlInsertAlt = "insert into alteration (alterationID, customerID, alteration, cost, totalCost, tailorDate, pickUpDate, notes, status, tailor) values ('$AltID','$ID','$aName','$aPrice','$totalCost','$tailorDate','$pickupDate','$notes','Altering','$tailorID')";
    if ($count > 0) {
        if (mysqli_query($con, $sqlInsertAlt)) {
            mysqli_query($con, "delete from tempalt where cusID=" . $ID);

            mysqli_query($con, "UPDATE `customerdetails` SET `alternationID` = '$AltID' WHERE `customerdetails`.`customerID` = '$ID'");
            mysqli_query($con, "update customerdetails set status = 'Altering',histroy  =  concat(histroy,',','Altering') WHERE `customerdetails`.`customerID` = '$ID'");
...
<head>
    ...
    <title><?php echo $name ?>'s Alteration</title>
    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse navbar-right" id="bs-example-navbar-collapse-1">
            <ul class="nav navbar-nav">
                <li class="active"><a href="#">    <?php echo $name ?>'s Alteration <span class="sr-only"></span></a>
                </li>
                <li class="dropdown">
...
<div class="container">
    ...
    <h4 class="text-center heading">Alteration Section</h4>
...
<!--     -->
            ...
            <div class="altTypes"><form method="post" class="text-center">
                <?php
        ...
        <form class="form-horizontal" method="post">
            <fieldset>
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-4">
                        <button id="submit" name="submit" class="btn palt">Submit Alterations</button>
                    </div>
                </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        $("#search").keypress(function () {
            $.ajax({
                type: 'POST',
                url: '../alteration/search.php?ID=ALT<?php echo $ID?>',
                data: {
                    name: $("#search").val(),
                },
...
<script>
    function comfirmDeleteCustomer() {
        return confirm("Are you sure, this will place the items in Section?");
    }
</script>
<script>
    function goBack() {
        return confirm("Are you sure, this will place the items in Section and save the alterations?");
    }
</script>
</body>
</html>