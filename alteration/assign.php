...
if (isset($_REQUEST['assign'])) {
    ...
    if ($tailor !== "None") {
        $getTailorsName = mysqli_query($con,"select * from tailor where tailorID = ".$tailor);
        while ($rowData = ($getTailorsName)->fetch_assoc()) {
            $tailorName = $rowData['tailor'];
        }
        $sqlPrintRecord = mysqli_query($con, "Select c.customerID,a.alterationID,c.customerName,c.customerNumber ,r.realtID from assigntemp join alteration a on assigntemp.alterationID = a.alterationID join customerdetails c on a.customerID = c.customerID left join realt r on assigntemp.realtID = r.realtID");
        if (mysqli_num_rows($sqlPrintRecord) > 0) {
            foreach ($sqlPrintRecord as $items) {
                if ($items['realtID'] === $items['alterationID']) {
                    $check = "RT";
                    mysqli_query($con, "update realt set status = 'Re-Altering', tailor = '$tailor' where realtID =" . $items['alterationID']);

                } else {
                    $check = "AT";
                    mysqli_query($con, "update alteration set status = 'Altering', tailor = '$tailor' where alterationID =" . $items['alterationID']);

                }
                mysqli_query($con, "update customerDetails set histroy  =  concat(histroy,',','Tailor Assigned: ','$tailorName') where customerID= " . $items['customerID']);
                mysqli_query($con, "delete from assigntemp where alterationID = " . $items['alterationID']);
...
if (isset($_REQUEST['delete'])) {
    ...
    $getReAlt = mysqli_query($con, "Select * from assigntemp where alterationID = {$_REQUEST['deleteID']}");
    ...
    $deleteTempAlt = "delete from assigntemp where alterationID = {$_REQUEST['deleteID']}";
    $deleteReTempAlt = "delete from assigntemp where realtID = {$_REQUEST['deleteID']}";
    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse navbar-right" id="bs-example-navbar-collapse-1">
            <ul class="nav navbar-nav">
                <li class="active"><a href="#">Assign Tailors<span class="sr-only"></span></a></li>
                <li class="dropdown">
...
<div class="container">
    <form class="form-horizontal" method="post"
          onsubmit="return confirm('The following customers will be assigned to the selected tailor.')">

        <h4 class="heading">Assign Tailor</h4>

        <div class="right">
            <a href="search.php">
                <button id="weddingParty" type="submit" name="assign" class="btn btn-lg">Assign
                </button>
            </a>
        </div>
        <br>
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            ...
            <table class="table table-hover">
                ...
                <tbody>
                <?php
                $sqlPrintRecord = mysqli_query($con, "Select * from assigntemp join alteration a on assigntemp.alterationID = a.alterationID join customerdetails c on a.customerID = c.customerID left join realt r on r.realtID = assigntemp.realtID");
                if (mysqli_num_rows($sqlPrintRecord) > 0) {
                    foreach ($sqlPrintRecord as $items) {
...
<script type="text/javascript">
    $(document).ready(function () {
        $("#search").keypress(function () {
            $.ajax({
                type: 'POST',
                url: '../alteration/search.php?ID=ASN',
                data: {
                    name: $("#search").val(),
                },
